...
func NewSessionID(signingKey string) (SessionID, error) {
	if len(signingKey) == 0 {
		err := fmt.Errorf("signingKey cannot be zero-length")
		return InvalidSessionID, err
	}

	cID := make([]byte, idLength)
	_, err := rand.Read(cID)
	if err != nil {
		return InvalidSessionID, err
	}
	...
	h.Write([]byte(cID))
	signature := h.Sum(nil)
	return SessionID(base64.URLEncoding.EncodeToString(append(cID, signature...))), nil
}
...
func ValidateID(id string, signingKey string) (SessionID, error) {
	token, err := base64.URLEncoding.DecodeString(id)
	if err != nil {
		return InvalidSessionID, fmt.Errorf("error base64-decoding: %v", err)
	}
	h := hmac.New(sha256.New, []byte(signingKey))
	h.Write([]byte(token[0:idLength]))
	sig := h.Sum(nil)

	if !hmac.Equal(sig, token[idLength:len(token)]) {
